...
def request_api(project_root,audio_path,AudioCount):
    # 服务端口和地址
    hps = utils.get_hparams_from_file(os.path.join(project_root,"configs/json/config.json"))
    ...
    if response.status_code == 200:
        # 将响应保存为文件
        with file_lock:
            with open(os.path.join(project_root, f"template/tts/{AudioCount}.wav"), 'wb') as f:
                f.write(response.content)
        return os.path.join(project_root, f"template/tts/{AudioCount}.wav")
    else:
